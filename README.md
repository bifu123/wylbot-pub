...
### 命名空间
命名空间是不同状态下不同语料背景的逻辑集合，比如在“插件问答”状态下，有命名空间“固定资产管理”、“记事本”，它们或者连接了本地业务系统、或者加载了本地文档，除了默认的“test”命名空间外，其余的都在插件中实现。比如，要切换到一个名为“记事本”的命名空间，发送“：：记事本”（：不分全角半角）即进入了该命名空间。元龙机器人是没有退出命名空间命令的，只能从一个命名空间切换到另一个命名空间，如果不想加载某个空间的功能和语料，可以使用“/聊天”切换到聊天状态，再用“：：test”切换到默认命名空间，直接与大模型对话。
