...
# 函数用于插入记录到db_path表
def insert_into_db_path(source_id, path, path_site):
    with db_lock:
        conn = get_database_connection()
        cursor = conn.cursor()
        cursor.execute('''INSERT OR REPLACE INTO db_path (source_id, path, path_site)
                          VALUES (?, ?, ?)''', (source_id, path, path_site))
        conn.commit()
...
def get_path_by_source_id(source_id):
    with db_lock:
        conn = get_database_connection()
        cursor = conn.cursor()
        cursor.execute('''SELECT path, path_site FROM db_path WHERE source_id = ?''', (source_id,))
        result = cursor.fetchone()
        if result:
            return result
        else:
